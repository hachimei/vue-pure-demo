...
// ---------------------IndoorMap2D class-----------------
const IndoorMap2d = function (mapdiv) {
  var _this = this
  var _mapDiv = mapdiv
  var _controls
...
const Canvas2DRenderer = function (map) {
  var _this = this
  var _map = map
  var _ctx
  ...
  this.updateViewport = function (isZoom) {
    ...
    _canvasHalfSize[0] = _canvasSize[0] * 0.5
    _canvasHalfSize[1] = _canvasSize[1] * 0.5
  ...
  this.focus = function (object) {
    ...
    minSize /= 2
    padding = padding < minSize ? minSize : padding
  ...
  this.screenShot = function (type) {
    ...
    tmpCanvas.width = _map.containerSize[0]
    tmpCanvas.height = _map.containerSize[1]
  ...
  this.render = function () {
    ...
    if (options.showNames) {
      ...
      for (var i = 0; i < funcAreas.length; i++) {
        ...
        //    }
        // }
        if ((funcAreas[i].rect.br[0] - funcAreas[i].rect.tl[0]) * 0.9 < nameText.halfWidth * 2) { nameText.visible = false }
        if (nameText.visible) {
    ...
    if (options.showPubPoints) {
      var pubPoints = _curFloor.PubPoint
      var imgWidth = 20; var imgHeight = 20
      //            if(_scale < 0.1){
      //                imgWidth = imgHeight = 12;
      //            }

      var imgWidthHalf = imgWidth / 2; var imgHeightHalf = imgHeight / 2
      var pubPointRects = []
      for (let i = 0; i < pubPoints.length; i++) {
        var pubPoint = pubPoints[i]
        center = pubPoint.newOutline
  ...
  function hitTest (point) {
    ...
    for (var i = 0; i < _curFloor.FuncAreas.length; i++) {
      ...
      if ((point[0] < rect.tl[0] && point[0] < -rect.br[1]) || (point[0] > rect.br[0] && point[0] > -rect.tl[1])) { continue }
...
const Controller2D = function (renderer) {
  ...
  this.endPoint = [0, 0]
  var _panVector = [0, 0]
  var _zoomDistStart = 0; var _zoomDistEnd = 0
  var _zoomScale = 1
